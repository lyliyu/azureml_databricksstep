...
def main(args):
    ...
    svm_model = SVC(kernel=args.kernel, C=args.penalty, gamma=0.001).fit(data['train']['X'], data['train']['y'])
...
def parse_args():
    ...
    parser.add_argument('--input_params_file', type=str, default='', help='File containing input params.')
    args, unknown = parser.parse_known_args()
    params = None
    if args.input_params_file:
        with open(args.input_params_file, 'r') as file:
            param_str = file.read().replace('\n', '')
            params = [s.strip() for s in param_str.split(" ")]
